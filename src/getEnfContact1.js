...
const  getEnfContact=require("./getEnfContact2.js")
...
async function getEnfContact1() {
	...
	for(const link of links) {
		console.log("总共还有"+links.length+"个安装商链接待处理！")
		...
		try {
				...
				console.log(`HTTP1 Status Code: ${status}`);

			if(status>=200&&status<300) {
					
					// console.log('Page1 loaded successfully');
					...
					if(regex.test(siteData.url&&siteData.email)) {
						...
						// 将格式化后的数据保存到文件
						fs.appendFileSync('data/获取的安装商联系信息.txt',formattedData);
...
// async function run() {
// 	await processLinks();  // 等待主方法完成
// 	await getEnfContact();     // 然后再调用另一个方法
// }


// run();

module.exports={ getEnfContact1 }; // 导出该函数
